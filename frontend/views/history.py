...
def render() -> None:
    ...
    st.markdown(f'<div class="page-title">{t("hist_title", lang)}</div>', unsafe_allow_html=True)
    st.markdown(
    ...
    if not history:
        st.info(t("hist_no_sims", lang))
        if st.button(t("hist_start_btn", lang), type="primary"):
            navigate("input")
        return
    ...
    with m1:
        st.markdown(f"""<div class="metric-card">
            <div class="mc-label">{t("hist_total_sims", lang)}</div>
            <div class="mc-value">{len(history)}</div>
            <div class="mc-sub">{t("hist_this_session", lang)}</div>
        </div>""", unsafe_allow_html=True)
    with m2:
        st.markdown(f"""<div class="metric-card">
            <div class="mc-label">{t("hist_best_ev", lang)}</div>
            <div class="mc-value" style="color:#2e7d32">{max(evs):.1f}</div>
            <div class="mc-sub">sc/ha</div>
        </div>""", unsafe_allow_html=True)
    with m3:
        st.markdown(f"""<div class="metric-card">
            <div class="mc-label">{t("hist_avg_ev", lang)}</div>
    ...
    with m4:
        ...
        st.markdown(f"""<div class="metric-card">
            <div class="mc-label">{t("hist_top_sim", lang)}</div>
    ...
    # ── Simulation list ────────────────────────────────────────────────────
    st.markdown(f'<div class="section-hdr">{t("hist_sims_run", lang)}</div>', unsafe_allow_html=True)
    ...
    for sim in reversed(history):
        ...
        <div class="hist-card">
            ...
            <div class="hist-main">
                ...
                <div class="hist-sub">{sim['timestamp']} &middot; {t("hist_best_path", lang)}: {opt_label}</div>
            </div>
            <div class="hist-ev">
                <div class="hist-ev-val">{ev_best:.1f}</div>
                <div class="hist-ev-unit">{t("hist_sc_ha_best", lang)}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)

        with st.expander(t("hist_details_expander", lang, sim_id=sim['id'])):
            dcol, rcol = st.columns(2)
            with dcol:
                st.markdown(f"**{t('hist_inputs', lang)}**")
                ...
                if display:
                    ...
                    st.caption(t("hist_no_display", lang))
            with rcol:
                st.markdown(f"**{t('hist_decision_criteria', lang)}**")
            ...
            <div style="display:flex;gap:16px;margin-top:12px">
                <div class="metric-card" style="flex:1;padding:12px 16px">
                    <div class="mc-label">{t("hist_your_ev", lang)}</div>
                    <div class="mc-value" style="font-size:1.3rem">{ev_user:.1f}</div>
                    <div class="mc-sub">sc/ha</div>
                </div>
                <div class="metric-card" style="flex:1;padding:12px 16px">
                    <div class="mc-label">{t("hist_best_avail_ev", lang)}</div>
                    <div class="mc-value" style="font-size:1.3rem;color:#2e7d32">{ev_best:.1f}</div>
                    <div class="mc-sub">sc/ha</div>
                </div>
                <div class="metric-card" style="flex:1;padding:12px 16px">
                    <div class="mc-label">{t("hist_gap", lang)}</div>
            ...
            if st.button(t("hist_restore_btn", lang), key=f"restore_{sim['id']}"):
    ...
    # ── Comparison chart ───────────────────────────────────────────────────
    if len(history) >= 2:
        st.markdown(f'<div class="section-hdr">{t("hist_comp_hdr", lang)}</div>', unsafe_allow_html=True)
        ...
        fig.add_hline(y=60, line_dash="dot", line_color="#999",
                      annotation_text=t("hist_baseline_label", lang), annotation_font_size=10)
        fig.update_layout(
            barmode="group",
            height=380,
            yaxis_title="EV (sc/ha)",
            xaxis_title=t("hist_sim_axis", lang),
        ...
        st.markdown(f'<div class="section-hdr">{t("hist_input_comp_hdr", lang)}</div>', unsafe_allow_html=True)
        comp_rows = {}
        all_keys = list(history[0].get("display", {}).keys())
        for k in all_keys:
            comp_rows[k] = [h.get("display", {}).get(k, "—") for h in history]
        comp_rows[t("hist_ev_best_col", lang)] = [f"{ev:.1f}" for ev in ev_best_l]
        comp_rows[t("hist_ev_user_col", lang)] = [f"{ev:.1f}" for ev in ev_user_l]
